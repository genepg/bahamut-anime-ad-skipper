...
- **Viewer Pause**: A pause that followed a trusted user gesture, as opposed to one a creative or the browser made. The keep-playing nudge never overrides one — that is what made the player look unpausable.
...
- **`MediaKeepalive` Module**: Shared nudge for paused ad videos. One implementation, two worlds — the page engine and every ad frame. Owns the **Viewer Pause** attribution too, since only the module that resumes a video can be trusted to know which pauses it must leave alone.
